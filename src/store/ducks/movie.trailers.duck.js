...
export function* saga() {
  yield takeLatest(actionTypes.MovieTrailerRequest, function* MovieTrailerSaga({
    payload,
  }) {
    try {
      const { response, error } = yield call(
        axios.get,
        `https://api.themoviedb.org/3/movie/${payload.id}?api_key=b0d65862c66030895d7983da2bd70edd&append_to_response=videos,images`
      );
